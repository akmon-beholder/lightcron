...
Feature: Detect and Recover Stuck Jobs via Two-Stage Liveness Check
  ...
  #     /health fails   → Stage 2: mark worker offline, mark jobs lost
  ...
  Scenario: Worker with stale last_seen and failing /health is marked offline
    ...
    And job "j-002" status in the jobs table is "lost"

  # ── Stage 2: threshold exceeded, skip /health ────────────────────────────────

  @smoke
  Scenario: Running job is marked lost when last_seen exceeds 90s (no /health probe)
    ...
    And job "j-003" status in the jobs table is "lost"

  @smoke
  Scenario: Assigned job is marked lost when last_seen exceeds 90s
    Given a worker_status row for "w-002" has last_seen 90 seconds ago
    And a job "j-004" in the jobs table has status "assigned" with worker_id "w-002"
    When the scheduler runs its health check
    Then job "j-004" status in the jobs table is "lost"
  ...
  @smoke
  Scenario: Lost jobs are visible via the REST API status filter
    Given a job "j-006" has status "lost" with worker_id "w-001" in the jobs table
  ...
  @smoke
  Scenario: Worker that re-registers after going offline recovers its worker_id and comes back online
    Given a worker_status row for hostname "worker-01" with worker_id "w-001" has status "offline"
    ...
    And the worker_id remains "w-001"
    And last_seen is updated to approximately now

  @smoke
  Scenario: Worker agent kills a process it is still running when the scheduler has marked it lost
    Given a worker_status row for "w-001" has status "online"
    And a job "j-007" in the jobs table has status "lost" with worker_id "w-001"
    And the worker_agent on "w-001" still has the process for "j-007" running locally
    When the worker_agent polls the jobs table for its running job statuses
    And detects that "j-007" has status "lost"
    Then the worker_agent kills the process for "j-007"
    And the worker_agent does not write any status update to the jobs table for "j-007"

  # ── Mass failure ─────────────────────────────────────────────────────────────

  @error-handling
  Scenario: Multiple workers failing simultaneously marks all their jobs as lost
    ...
    When the scheduler runs its health check
    Then job "j-007" status is "lost"
    And job "j-008" status is "lost"
