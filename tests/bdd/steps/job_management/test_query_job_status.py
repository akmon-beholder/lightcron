...
def given_running_job_with_worker(ctx: SimpleNamespace) -> None:
    ...
    job_id = db_run(insert_job, command="/usr/bin/my-script.sh", status="running", worker_id=wid)
    ctx.job_ids["j-001"] = job_id


@given("jobs exist with statuses \"pending\", \"running\", and \"completed\"")
def given_mixed_status_jobs(ctx: SimpleNamespace) -> None:
    for status in ("pending", "running", "completed"):
        db_run(insert_job, status=status)


@given("3 jobs exist with varying statuses")
def given_three_jobs(ctx: SimpleNamespace) -> None:
...
def get_job_j001(ctx: SimpleNamespace, http_client: TestClient) -> None:
    job_id = ctx.job_ids.get("j-001")
    url = f"/jobs/{job_id}" if job_id else "/jobs/00000000-0000-0000-0000-000000000000"
    ctx.response = http_client.get(url)


@when("GET /jobs/does-not-exist is called")
def get_unknown_job(ctx: SimpleNamespace, http_client: TestClient) -> None:
    ctx.response = http_client.get("/jobs/00000000-0000-0000-0000-000000000000")


@when("GET /jobs?status=running is called")
def get_jobs_running(ctx: SimpleNamespace, http_client: TestClient) -> None:
    ctx.response = http_client.get("/jobs", params={"status": "running"})


@when("GET /jobs is called")
def get_all_jobs(ctx: SimpleNamespace, http_client: TestClient) -> None:
    ctx.response = http_client.get("/jobs")


@when("GET /jobs?status=not-a-real-status is called")
def get_jobs_invalid_status(ctx: SimpleNamespace, http_client: TestClient) -> None:
    ctx.response = http_client.get("/jobs", params={"status": "not-a-real-status"})
