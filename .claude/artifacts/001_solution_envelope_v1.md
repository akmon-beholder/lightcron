...
Lightcron is a lightweight job orchestration system. Users schedule jobs via a REST API, specifying a command to run, a start time, optional resource limits, and optional dependencies on other jobs. Worker agents running on separate nodes poll a shared PostgreSQL database for jobs to claim and execute. The scheduler service maintains job lifecycle state and worker liveness. There is no message broker — the database is the sole communication bus between all system components.
...
| Role | Description |
|---|---|
| `job_submitter` | Calls the scheduler REST API to schedule, query, and cancel jobs |
| `platform_operator` | Manages the Lightcron deployment and worker fleet |
| `sre_operator` | Responds to incidents; needs API-level visibility and control |
...
Both units are Python packages within the same repository. They share domain model definitions but have separate entry points and dependency sets.
...
| # | Question | Needed by |
|---|---|---|
| OQ-01 | What are the poll intervals for the claim loop, ready-transition loop, and job-status-check loop? | Spec |
| OQ-02 | What is the SIGTERM grace period before SIGKILL? | Spec |
| OQ-03 | Should dependency cycles be rejected at submission or detected at runtime? | Spec |
| OQ-04 | Should `assigned` jobs orphaned by a worker_agent restart be retried (back to `ready`) or failed? | Spec |
| OQ-05 | Max jobs per worker — is there a configurable concurrency limit per worker_agent? | Spec |
...
- `002_spec_v1.md` — Full functional spec covering all 8 journeys and 5 open questions
- `003_tasklist_v1.md` — Implementation tasks ordered per suggested sequence (J002 → J001 → J003 → J004 → J008 → J005 → J006 → J007)

**Must include in spec:**
- Resolution of all 5 open questions (OQ-01 through OQ-05)
- All 6 ops conditions (C1–C6) as explicit tasks in the tasklist
- Worker agent startup recovery (Gotcha #1) as a task
- Cancel/lost race condition guard (Gotcha #5) as an acceptance criterion
- Dependency cycle handling (Gotcha #6) — decision required first (OQ-03)
...
10. BDD step definitions (pytest-bdd)
11. CI pipeline (gitleaks, alembic check, FK integrity, coverage)
